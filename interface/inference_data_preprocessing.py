...
def add_ev_power(df, static_path = "../data/excel_files/2025_7-8.xlsx", ev_path = "../data/additional_data/EV_chargers.xlsx"):
    ...
    ev_data["ev_start_utc"] = pd.to_datetime(ev_data["ev_start"])
    ...
    # Strip timezone from both sides before comparing — the weather pipeline
    # may leave df["datetime"] tz-aware (UTC) while ev_start is tz-naive, or
    # vice versa. tz_convert(None) removes tz from aware series; tz-naive
    # series are returned unchanged via the identity branch.
    def _naive(s: pd.Series) -> pd.Series:
        return s.dt.tz_convert(None) if isinstance(s.dtype, pd.DatetimeTZDtype) else s

    mask_installed = (
            merged["ev_start_utc"].isna() |
            (_naive(merged["ev_start_utc"]) <= _naive(merged["datetime"]))
    )
...
no_weather_path = "../data/inference/bronze/no_weather.parquet"
weather_path = "../data/inference/bronze/with_weather.parquet"
checkpoint_path = "../data/inference/bronze/with_weather.parquet"

silver_path = "../data/inference/silver/feature_engineered_built_idx_split.parquet"

gold_folder = "../data/inference/gold"

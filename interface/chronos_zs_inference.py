"""
Inference script for Chronos-2 in **zero-shot** mode.

Identical preprocessing, regularization, and rolling logic to
`inference_chronos.py`, but loads the pretrained `amazon/chronos-2` weights
directly from HuggingFace instead of a local fine-tuned checkpoint. Use this
as a baseline to compare against the fine-tuned model.

Public API:
    predict_chronos_zeroshot(df, start_dt, end_dt) -> pd.DataFrame
"""
...
# -----------------------------------------------------------------------------
# Pipeline loader (cached so repeat calls don't re-download / re-init weights)
# -----------------------------------------------------------------------------
_PIPELINE_CACHE: Dict[str, Chronos2Pipeline] = {}


def _load_pipeline(model_id: str = MODEL_ID) -> Chronos2Pipeline:
    """Load the pretrained Chronos-2 pipeline from HuggingFace.

    First call downloads weights into the HF cache (~/.cache/huggingface),
    subsequent calls reuse the cached files. The pipeline object itself is
    also cached in-process so a single Python session loads weights once.
    """
...
# -----------------------------------------------------------------------------
def predict_chronos_zs(df: pd.DataFrame, start_dt: str, end_dt: str) -> pd.DataFrame:
    """Run rolling Chronos-2 inference in **zero-shot** mode on the unknown-target
    portion of df.

    Same signature, preprocessing, and return shape as `predict_chronos` /
    `predict_xgboost` — only difference is the model: pretrained weights from
    `amazon/chronos-2` with no task-specific fine-tuning.
    ...
    print(f"Loading Chronos-2 pipeline (zero-shot, {MODEL_ID})...")
...
if __name__ == "__main__":
    ...
    df_preds = predict_chronos_zs(df, start_dt="2025-09-01", end_dt="2025-10-30")
    print(df_preds)